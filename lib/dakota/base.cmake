...
dk_append_target_property (${target} LINK_FLAGS --cxx ${cxx-compiler} --parts ${parts} --var=source_dir=${CMAKE_CURRENT_SOURCE_DIR})
target_compile_options (   ${target} PRIVATE    --cxx ${cxx-compiler} --parts ${parts} --var=source_dir=${CMAKE_CURRENT_SOURCE_DIR})
...
execute_process (
  COMMAND ${CMAKE_CXX_COMPILER} --target-src --path-only --parts ${parts} --var=source_dir=${CMAKE_CURRENT_SOURCE_DIR}
...
add_custom_command (
  OUTPUT ${target-src}
  COMMAND ${CMAKE_CXX_COMPILER} --target-src --parts ${parts} --var=source_dir=${CMAKE_CURRENT_SOURCE_DIR}
...
add_custom_target (
  ${target-hdr}
  DEPENDS ${parts} ${target-libs} dakota-catalog${CMAKE_EXECUTABLE_SUFFIX}
  COMMAND ${CMAKE_CXX_COMPILER} --target-hdr --parts ${parts} --var=source_dir=${CMAKE_CURRENT_SOURCE_DIR}
  VERBATIM)
