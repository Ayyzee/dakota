...
dk_append_target_property (${target} LINK_FLAGS --cxx ${cxx-compiler} --var=build_dir=${current-build-dir} --var=source_dir=${CMAKE_CURRENT_SOURCE_DIR})
target_compile_options (   ${target} PRIVATE    --cxx ${cxx-compiler} --var=build_dir=${current-build-dir} --var=source_dir=${CMAKE_CURRENT_SOURCE_DIR})
...
execute_process (
  COMMAND ${CMAKE_CXX_COMPILER} --target-src --path-only --var=build_dir=${current-build-dir}
...
add_custom_command (
  ...
  COMMAND ${CMAKE_CXX_COMPILER} --target-src --var=build_dir=${current-build-dir} --var=source_dir=${CMAKE_CURRENT_SOURCE_DIR}
  VERBATIM)
...
add_custom_target (
  ${target-hdr}
  DEPENDS ${parts} ${target-libs} dakota-catalog${CMAKE_EXECUTABLE_SUFFIX}
  COMMAND ${CMAKE_CXX_COMPILER} --target-hdr --var=build_dir=${current-build-dir} --var=source_dir=${CMAKE_CURRENT_SOURCE_DIR}
  VERBATIM)
