...
sub src_path {
  ...
  if ($ENV{'DK_ABS_PATH'}) {
    my $cwd = &getcwd();
    return "$cwd/$current_intmd_dir/$name.$ext";
  } else {
    return "$current_intmd_dir/$name.$ext";
  }
}
...
sub generate_target_runtime {
  ...
  $target_src_str .= "[[read-only]] static symbol-t type = dkt-file-type-containing-addr(cast(ptr-t)&zero);" . $nl;
  $target_src_str .= $nl;
  #my $col;
