...
sub generics::generate_generic_defn {
  ...
  if (&is_nrt_decl() || &is_rt_decl()) {
    ...
    $$scratch_str_ref .= "_func_($$new_arg_names_list);\n";
    if ($big_generic) {
      if ($ENV{'DK_ENABLE_TRACE_MACROS'}) {
        my $result = 'result';
        if ($$arg_names_list =~ m/,/) {
...
sub generate_kw_args_method_defn {
  ...
  $$scratch_str_ref .=
    "$klass_type @$klass_name { namespace va { " . $visibility . $func_spec . "KW-ARGS-METHOD $method_name($$new_arg_list) -> $return_type {" . &ann(__FILE__, __LINE__) . "\n";
  $col = &colin($col);
