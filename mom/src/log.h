...
#define MOM_LOG_WARNING_DL() MOM_LOG_WARNING("%s:%i: warning: %s() \"%s\"",  __FILE__, __LINE__, __func__,  sys::dlerror())
...
#if 1
#define MOM_LOG_SND_MSG_STMT(stmt) stmt
#define MOM_LOG_SND_MSG(...) MOM_LOG(mom::LOG_SND_MSG_FLAG, __VA_ARGS__)
#else
#define MOM_LOG_SND_MSG_STMT(stmt)
