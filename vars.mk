...
ifdef DKT_PROFILE
  DAKOTA ?= DK_ECHO_LINK_CMD=0 DK_NO_LINE=0 DK_NO_CONVERT_DASH_SYNTAX=0 DK_USE_TARGET_HEADER=0 $(DAKOTA-PROFILE-BASE)
  # prof uses -p, gprof uses -pg
  EXTRA_CXXFLAGS += -pg
  EXTRA_LDFLAGS  += -pg
else
  DAKOTA ?= DK_ECHO_COMPILE_CMD=0 DK_ECHO_LINK_CMD=0 DK_NO_LINE=0 DK_NO_CONVERT_DASH_SYNTAX=0 DK_USE_TARGET_HEADER=0 $(DAKOTA-BASE)
  # --keep-going
endif
